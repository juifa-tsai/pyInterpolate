...
## * loading model 
#OK = PKL.load(open('/Volumes/Data2/Workspace/GENM/results/model/interpolation_model_sticky_expected_user_2016-11_x1625_y1625_r0.pkl', 'rb'))
...
xy = NP.concatenate(( X.flatten()[:, NP.newaxis] + TW_CELLWIDTH/2, 
                      Y.flatten()[:, NP.newaxis] - TW_CELLWIDTH/2), 
                    axis=1)
print(xy)
xbins = X.shape[1]
ybins = Y.shape[0]
...
print('>>        n_neighbor: %d, radius : %.2f (m)'%(N_NEIGHBOR, OK.variogram().get_params()[1]))
print(OK.variogram().get_params()[1])
batches= TQDM(batches)
for idx in batches:
    batches.set_description(">> ")
    z_new, ze_new = OK.predict( X = xy[idx[0]:idx[1], :], 
                                n_neighbor = N_NEIGHBOR,
                                radius = OK.variogram().get_params()[1],
                                get_error = True
                              )
    z = NP.append(z, z_new)
...
transform = from_origin(X.min(), Y.max(), TW_CELLWIDTH, TW_CELLWIDTH) # Set raster left-top's coordinates
